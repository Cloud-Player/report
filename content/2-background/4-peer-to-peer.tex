...
The second mechanism they have in place is called \say{bootstrap-list}. The \say{bootstrap-list} is a set of addresses a client can connect to. If one fails it tries the next one. As soon as the client is in the network it stores discovered peer addresses in a local database. The next time the client is using also using those addresses to enter the network.
